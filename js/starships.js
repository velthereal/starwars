...
function showAllPerson(data){
	...
	data.forEach(element => {
		let str = `<div class="card mb-3">
			...
			<img src="https://starwars-visualguide.com/assets/img/starships/${element.url.match(/\/([0-9]*)\/$/)[1]}.jpg" class="d-block user-select-none onerror="myFunction()"></div>`;
			content.insertAdjacentHTML('beforeend', str);
	});
	showPerson(data);
}
function myFunction(){
	let img = document.querySelectorAll('.d-block');
	for(let i = 0; i < img.length; i++){
		if(img[i].readyState === 404){
			img[i].src = 'https://starwars-visualguide.com/assets/img/placeholder.jpg';
		}
	}
}
